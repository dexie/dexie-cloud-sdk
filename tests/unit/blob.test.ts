...
describe("BlobManager", () => {
  ...
  describe("upload", () => {
    ...
    it("throws when server returns empty body (version cannot be determined)", async () => {
      ...
      await expect(
        manager.upload(uint8([1]), "application/octet-stream", TOKEN),
      ).rejects.toThrow(DexieCloudError);
    });
    ...
    it("accepts ArrayBuffer input", async () => {
      ...
      await expect(
        manager.upload(buf, "application/octet-stream", TOKEN),
  ...
  describe("download", () => {
    ...
    it("throws on download error", async () => {
      ...
      await expect(manager.download("1:missing", TOKEN)).rejects.toThrow(
        DexieCloudError,
      );
    });
  });
  ...
  describe("processForUpload (auto mode)", () => {
    ...
    it("sets _hasBlobRefs=1 on result when blobs were offloaded", async () => {
      fetchMock.mockResolvedValue(mockJsonResponse({ ref: "1:uploaded" }));

      const bigB64 = btoa(String.fromCharCode(...new Array(4096).fill(0)));
      const obj = {
        name: "test",
        file: { _bt: "Blob", v: bigB64, ct: "image/png" },
      };

      const result = await manager.processForUpload(obj, TOKEN);
      expect(result._hasBlobRefs).toBe(1);
    });

    it("does NOT set _hasBlobRefs when no blobs were offloaded", async () => {
      const obj = { id: "1", name: "plain" };
      const result = await manager.processForUpload(obj, TOKEN);
      expect(result._hasBlobRefs).toBeUndefined();
    });
  });
